...
--   streak_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
--   user_id UUID NOT NULL REFERENCES user_profile(id) ON DELETE CASCADE,
--   session_id UUID NOT NULL REFERENCES quiz_sessions(session_id) ON DELETE CASCADE,
--   streak_value INT NOT NULL CHECK(streak_value >= 0),
...
WITH continent_counts AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE cou.type = 'country') AS countries_count,
    COUNT(*) FILTER (WHERE cou.type = 'territory') AS territories_count
  FROM continents con
  JOIN country_continent cc ON con.continent_id = cc.continent_id
  JOIN countries cou ON cc.country_id = cou.country_id
  GROUP BY con.continent_name
),
world_counts AS (
  SELECT
    'World'::TEXT AS region,
    (SELECT COUNT(DISTINCT country_id) FROM countries WHERE type = 'country') AS countries_count,
    (SELECT COUNT(DISTINCT country_id) FROM countries WHERE type = 'territory') AS territories_count
)

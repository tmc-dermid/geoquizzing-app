...
--   correct_ratio FLOAT GENERATED ALWAYS AS (
--     CASE
--       WHEN total_correct_answers + total_incorrect_answers = 0 THEN 0
--       ELSE total_correct_answers::FLOAT / (total_correct_answers + total_incorrect_answers)
--     END
--   ),
--   incorrect_ratio FLOAT GENERATED ALWAYS AS (
--     CASE
--       WHEN total_correct_answers + total_incorrect_answers = 0 THEN 0
--       ELSE total_incorrect_answers::FLOAT / (total_correct_answers + total_incorrect_answers)
--     END
--   ),
...
--     points_earned INT GENERATED ALWAYS AS (
--         GREATEST(0, (2 * total_correct) - used_hints_count)
--     ) STORED,
...
--     is_correct BOOLEAN,
--     hint_used BOOLEAN,
--     answer_time INT USING EXTRACT(EPOCH FROM answer_time)::INT,
...
UPDATE questions
SET question_img = c.flag_url
FROM countries c
WHERE questions.country_id = c.country_id
  AND questions.question_id BETWEEN 263 AND 494;
...
SELECT
  ...
  correct_answer
FROM questions
WHERE question_id > 741
ORDER BY question_id ASC;

SELECT
  c.country_name,
  c.highest_point_name,
  con.continent_name
FROM countries c
JOIN country_continent cc USING(country_id)
JOIN continents con USING(continent_id)
WHERE c.type = 'territory'
ORDER BY 1 ASC;
